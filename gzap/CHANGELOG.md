...
⚠️ Logic and tuning format has changed. Logic files must be re-generated. Tuning
files generated by old versions are invalidated. Tuning files generated by recent
versions may need to have entries for secrets repaired in order to load.

⚠️ This update splits the apworld. The core apworld (`gzdoom.apworld`) contains
the randomizer itself and logic for the games listed as "Core WADs" in the
[support table](doc/support-table.md). Logic for other WADs has been split into
[individual apworlds per wad](../release/apworlds). You should download and
install **only the apworlds for the wads you are actually playing**; installing
too many can result in Archipelago client errors when attempting to join a game.
