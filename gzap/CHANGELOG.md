...
- New:
  ...
  - Logic and tuning for `Faithless` episode 2. Chests are not randomized (neither the items in the chests themselves, nor items in hidden areas that open when the chest is unlocked), but none of these are part of progression in any case.
- Fixed:
