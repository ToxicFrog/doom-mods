...
class StartingLevels(OptionSet):
    ...
    Sayeth's (Logic maker) note - 
    BTSX e2 is separated into 5 mini-episodes each containing 3-6 levels.
    Depending on how long you want to play, I would recommend by default to
    keep the five default starting points per their mini-episode for mid-size
    playthroughs and to give you more variable access to checks, but
    feel free to reduce the amount of starting maps depending on the other 
    people's multiworlds. 
    You can also start with nothing but MAP01, it contains enough checks even 
    when starting without keycards.
    """
...
if wad.get_flag('use_hub_logic'):
    ...
    class StartWithKeys(Toggle):
        ...
        Sayeth's (Logic maker) note - 
        Highly recommend keeping this off. BTSX e2 has many large non-linear 
        levels, even if playing with just the first level enabled as starting
        map, it contains a lot of items and secrets accessible without any keys.

        Especially so if you start with all mini-episode starting levels 
        as per default, getting all the keys for 5 levels right from the 
        start will be overwhelming, especially for multiworlds with other people.
        """
        default = False
else:
    class StartWithKeys(Toggle):
...
class IncludedItemCategories(OptionList):
    ...
    You can also use two special values in place of a percentage:
      ...
      start with all keys, weapons, or maps, respectively.
        ...
        Sayeth's (Logic maker) note - 
    All powerups, large healing items, weapons, secrets and keys checks
    have been given custom names. If you play with hints, hinting for any of
    the aforementioned checks should give you a better idea where to look 
    for said check, as well as info if they require keys or secrets.
    """
    display_name = "Included item/location categories"
    default = [
        'secret-sector:none', 'secret-marker:none', 'secret:none',
        'ap_progression:all', 'ap_useful:all', 'big-health:all', 'armor:none',
...
class WinMapNames(OptionSet):
    ...
    levels.
        ...
        Sayeth's note - 
    BTSX e2 is separated into 5 mini-episodes each containing 3-6 levels. 
    My recommendation is to put either finales of each episode as Winmap - this means: 
    'MAP04', 'MAP10', 'MAP15', 'MAP22', 'MAP26' and/or the secret 'MAP31' which is a 
    challenging Slaughtermap, and can be used instead for a potential climactic ending.
    By default, I put MAP26 and MAP31, but please be aware that MAP31 is VERY 
    difficult, I don't recommend playing it without respawns/persistent mode enabled.
    """
...
class FullPersistence(Toggle):
    ...
    unless you die and respawn. Its marked in logic as inaccessible to not force 
...
class PreTuningMode(Toggle):
    ...
    intermission text, etc.
        ...
        Sayeth's note - 
    Not sure where else to put it - the list of all maps with locations marked 
    inaccessible. There are a couple places that are either secrets but not
    marked as such, you may only have one attempt to reach or are otherwise
    difficult to reach:
    MAP02 - one attempt jump, MAP04 - unmarked secret dark door, 
    MAP19 - unmarked secret close to rocket launcher,
    MAP24 - archvile jump BFG, MAP25 - Multi-zap jump area in BFG arena.
    If you have a check inside one of these maps/places, you can safely ignore it.
    """
