...
2. Download AceCorp Launcher and put it in the same directory where you extracted
   UZDoom, so that it is in the same folder as uzdoom.exe.
...
5. Download the UZDoom AP mod and place it in the same directory as uzdoom.exe.
...
1. Install `uzdoom` and `qzdl` via your package manager.
2. Create the config directory: `mkdir -p ~/.config/uzdoom`.
