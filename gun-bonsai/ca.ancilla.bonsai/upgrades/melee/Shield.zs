...
class ::Shield : ::BaseUpgrade {
  ...
  override void GetTooltipFields(Dictionary fields, uint level) {
    ...
    fields.insert("attackttl", string.format(StringTable.Localize("$TFLV_TT_SECONDS_REAL"), AttackTTL(level)/35.0));
    fields.insert("killttl", string.format(StringTable.Localize("$TFLV_TT_SECONDS_REAL"), KillTTL(level)/35.0));
  }
}
